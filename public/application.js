...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSignIn = function onSignIn(event) {
	  ...
	  api.signIn(data).then(function (response) {
	    ...
	    api.getGamesForUser().then(function (gamesResponse) {
	      // gets all games saved to that user
	      store.games = gamesResponse.games; // this saves the games to use later

	      store.games.forEach(function (game) {
	        // will go through each past game and determine the winner if any
	...
	var startNewGame = function startNewGame() {
	  if (store.user !== undefined) {
	    // checks if we have a user to get games for before moving on
	    ...
	    // make gameboard blank if user sign outs
	    ui.turnChange('X'); // Let the UI know that 'X' is the starting player
	    board.resetBoard();
	    ui.deleteOldGameTiles();
	    ui.startNewGame();
	...
	var updateGame = function updateGame(board, index, endGame) {
	  // split code into 2nd function to make easier to read
	...
	// CLICKING ACTION
	var clickedSpace = function clickedSpace() {
	  var id = this.getAttribute('data-id'); // gets the data-id off the gameboard div
	  if (board.isTileAvailable(id)) {
	    // checks if div is available or already marked
	    board.makeMove(id); // if available makes move onto the data id in the div
	    if (board.isFirstMoveMade() === false) {
	      // if the game's first move has not been made do this
...
/***/ function(module, exports, __webpack_require__) {
	...
	var getGamesForUser = function getGamesForUser() {
	  return $.ajax({
	...
	var createGamesForUser = function createGamesForUser() {
	  return $.ajax({
	    url: config.apiOrigin + '/games',
	    method: 'POST',
	    headers: {
	...
	var saveGamesForUser = function saveGamesForUser(boardId, move, value, gameOver) {
	  return $.ajax({
	...
	module.exports = {
	  ...
	  signOut: signOut,
	  getGamesForUser: getGamesForUser,
	  createGamesForUser: createGamesForUser,
	  saveGamesForUser: saveGamesForUser
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var prepareForNewGame = function prepareForNewGame() {
	  $("#reset-button").removeClass('hidden');
	};

	var turnChange = function turnChange(data) {
	  $('.winner-announcement').text(data + "'s turn!");
	};

	// is referenced by events startNewGame
	var deleteOldGameTiles = function deleteOldGameTiles() {
	  $('.tile').text('');
	};
	...
	var setCurrentGame = function setCurrentGame(data) {
	...
	var setGameHistory = function setGameHistory(data) {
	  $('#game-history-list').empty(); // gets rid of any games from other users
	  if (data.length === 0) {
	    // if no games played, displays this message
	    $('#game-history-list').text("Play some games so we can store them here!");
	  } else {
	    (function () {
	      // otherwise, we will list the games and make them clickable
	      $('#game-history-list').text(""); // clears the message if there is a game history
	      var gameCounter = 0; // using game counter to interate through all listed games so they each have a custom number in order to link them
	      data.forEach(function (game) {
	        // this iterates through the list and sets up the messages
	        var completed = "is in progress"; // default message is in progress until over=true or null

	        if (game.over === true) {
	          // if over=true...
	          if (game.winner === null) {
	            // and over=null then set message
	            completed = "ended in a draw";
	          } else if (game.winner === 'X') {
	            // or over=true and winner is x set message
	            completed = "was won by X";
	          } else if (game.winner === 'O') {
	            // or over=true and winner is o set message
	      ...
	      $('.restore-game').click(function () {
	        ...
	        api.getGamesForUser().then(function (gamesResponse) {
	          // gets all games saved to that user
	          var id = _this.getAttribute('data-id'); // gets the data-id from 136
	          var game = store.games[id]; // gets the stored game
	          ...
	          var tiles = $('.tile'); // GET LIST OF ALL GAME TILES FROM HTML
	          for (var i = 0; i < tiles.length; i++) {
	            // this sets up the visual board
	            var value = board.getTileValue(i); // GETS VALUE FROM THE BOARD (line 140)
	            var tile = tiles[i]; // gets each individual tile on the screen
	          ...
	          if (board.stillPlayingGame()) {
	            // if game was NOT over, don't show the new game button
	            $("#reset-button").addClass('hidden');
	          }
	...
	module.exports = {
	  ...
	  deleteOldGameTiles: deleteOldGameTiles,
	  startNewGame: startNewGame,
	  setCurrentGame: setCurrentGame,
	  setGameHistory: setGameHistory,
	  prepareForNewGame: prepareForNewGame
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports) {
	...
	var gameBoard = ['', '', '', '', '', '', '', '', ''];
	var stillPlaying = true; // to stop move after cats game or win
	var firstMoveMade = false; // reference to allow created new saved game
	var currentPlayer = 'X'; // x is always the first player

	var winnerCallback = function winnerCallback() {}; // Called as a function when the game is drawn or has a winner
	var turn = function turn() {}; // Called as a function when each turn is over

	var id = 0; // the id of the board on the server

	var setWinnerFunction = function setWinnerFunction(winner) {
	  // Saves a callback we can call when the game is over
	  winnerCallback = winner;
	};

	var setTurn = function setTurn(callback) {
	  // Saves a callback we can call at the end of each turn
	  turn = callback;
	};

	var isTileAvailable = function isTileAvailable(index) {
	  // Checks if the game is still running and if the array index is '' (no move made there yet)
	  return stillPlaying && gameBoard[index] === '';
	};

	var getTileValue = function getTileValue(index) {
	  // Gets the current value of the tile
	  return gameBoard[index];
	};

	// Provided function from fundamentals
	var allThree = function allThree(player, cellOne, cellTwo, cellThree) {
	...
	var winnerIs = function winnerIs(player) {
	  // ROWS BELOW
	  return allThree(player, getTileValue('0'), getTileValue('1'), getTileValue('2')) || allThree(player, getTileValue('3'), getTileValue('4'), getTileValue('5')) || allThree(player, getTileValue('6'), getTileValue('7'), getTileValue('8')) ||
	  // COLUMNS BELOW
	  allThree(player, getTileValue('0'), getTileValue('3'), getTileValue('6')) || allThree(player, getTileValue('1'), getTileValue('4'), getTileValue('7')) || allThree(player, getTileValue('2'), getTileValue('5'), getTileValue('8')) ||
	  // DIAGONALS BELOW
	  allThree(player, getTileValue('0'), getTileValue('4'), getTileValue('8')) || allThree(player, getTileValue('2'), getTileValue('4'), getTileValue('6'));
	};

	// Checks if the tile is not blank for isDraw for cat's game determination
	var checkTile = function checkTile(tile) {
	  return tile !== '';
	};
	...
	var makeMove = function makeMove(index) {
	  ...
	  if (winnerIs(currentPlayer)) {
	    ...
	    // If no one has won, switch to the next players turn
	    if (currentPlayer === 'X') {
	      currentPlayer = 'O';
	    } else {
	      currentPlayer = 'X';
	...
	var setUpBoardForGame = function setUpBoardForGame(game) {
	  // when clicking on a prior game this loads that game onto the shown game div
	  ...
	  var moveCount = 0; // moveCount starts at 0
	  gameBoard.forEach(function (cell) {
	    if (cell === 'X' || cell === 'O') {
	      // if the cell has an X or O...
	...
	var currentTurnPlayer = function currentTurnPlayer() {
	  // making this an exportable variable
	  return currentPlayer;
	};

	var stillPlayingGame = function stillPlayingGame() {
	  return stillPlaying; // created because variable wasn't exporting correctly
	};

	var isFirstMoveMade = function isFirstMoveMade(moveMade) {
	  // made becase variable not being exported correctly
	  if (moveMade !== undefined) {
	    // if moveMade was sent...
	...
	var determineWinner = function determineWinner(game) {
	...
	module.exports = {
	  ...
	  winnerIs: winnerIs,
	  getWinner: getWinner,
	  setWinnerFunction: setWinnerFunction,
	  setTurn: setTurn,
	  resetBoard: resetBoard,
	  isFirstMoveMade: isFirstMoveMade,
	  id: id,
	  setUpBoardForGame: setUpBoardForGame,
	  determineWinner: determineWinner,
	  stillPlayingGame: stillPlayingGame,
